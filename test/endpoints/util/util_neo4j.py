...
class UtilNeo4j:
    ...
    def create_user(self, user_id, name):
        ...
        with driver_neo4j.session() as session:
            session.write_transaction(
                self._create_user_neo4j, user_id, name)
    ...
    def create_type_games(self, type_games):
        ...
        with driver_neo4j.session() as session:
            for type_game in type_games:
                session.write_transaction(
                    self._create_type_game, type_game)

    @staticmethod
    def _create_type_game(tx, type_game):
        tx.run(SENTENCE_CREATE_TYPE_GAME.format(
            type_game[NAME],
            type_game[CODE],
    ...
    def create_relationship_user_typegame(self, user_id, type_games):
        ...
        with driver_neo4j.session() as session:
            for type_game in type_games:
                session.write_transaction(
                    self._create_relationship_user_typegame_neo4j, user_id, type_game[NAME])

    @staticmethod
    def _create_relationship_user_typegame_neo4j(tx, user_id, type_game_name):
        tx.run(SENTENCE_CREATE_RELATIONSHIP_TYPE_GAME_USER.format(
            user_id, type_game_name))
        ...
        with driver_neo4j.session() as session:
            for type_game in type_games:
                session.write_transaction(
                    ...
                    user_id, type_game[NAME], type_game[POINTS])

    @staticmethod
    def _create_relationship_played_user_typegame_neo4j(tx, user_id, type_game_name, points):
        tx.run(SENTENCE_CREATE_RELATIONSHIP_PLAYED_TYPE_GAME_USER.format(
